/*:
 * @plugindesc v1.1.0 Отображение здоровья врагов (Universal) [ACTPO]
 * @author ACTPOJIuT
 * 
...
(function () {
    ...
    var parameters = PluginManager.parameters('ACTPO_EnemyHP');

    // Конфигурация отображения
    var config = {
        // Основные настройки
        showHPBars: parameters['Показывать HP Бары'] === 'true',
        showHPText: parameters['Показывать HP Текст'] === 'true',

        // Размеры
        barWidth: Number(parameters['Ширина HP Бара']) || 100,
        barHeight: Number(parameters['Высота HP Бара']) || 8,

        // Позиционирование
        barYOffset: Number(parameters['Смещение Y от спрайта']) || -50,
        textYOffset: Number(parameters['Текст Y смещение']) || -65,
        fontSize: Number(parameters['Размер шрифта']) || 14,

        // Цвета
        colors: {
            hpBarBack: parameters['Цвет фона HP Бара'] || '#000000',
            hpBarFill: parameters['Цвет заполнения HP Бара'] || '#ff0000',
            hpText: parameters['Цвет текста HP'] || '#ffffff'
        },

        // Дополнительные настройки
        enableAnimations: parameters['Включить анимации'] === 'true',
        autoHideDelay: Number(parameters['Задержка скрытия (секунды)']) || 0,
        showOnlyOnDamage: parameters['Показывать только при уроне'] === 'true'
    };
    ...
    enemySpriteClasses.forEach(function (className) {
        ...
        if (spriteClass) {
            ...
            console.log('ACTPO_EnemyHP: Применен к классу ' + className);
        } else {
            console.log('ACTPO_EnemyHP: Класс ' + className + ' не найден');
        }
    });
