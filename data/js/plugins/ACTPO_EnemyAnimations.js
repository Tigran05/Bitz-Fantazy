/*:
 * @plugindesc v1.0.0 - ACTPO Система анимации врагов (SV спрайты + эффекты) [ACTPO]
 * @author ACTPOJIuT
 *
 * @param Debug
 * @text Режим отладки
...
Imported.ACTPO_EnemyAnimations = true;
...
ACTPO.EA.Parameters = PluginManager.parameters('ACTPO_EnemyAnimations');
...
ACTPO.EA.log = function (message) {
    if (ACTPO.EA.Debug) {
        console.log('[ACTPO_EnemyAnimations] ' + message);
    }
};
...
Game_Enemy.prototype.initACTPOAnimations = function () {
    ...
    // Проверяем <sv animated> или <sv character>
    if (note.match(/<sv animated>/i)) {
        this._svAnimated = true;
        this._isCharacter = false;
        this._hasShadow = true;
        ACTPO.EA.log(enemy.name + ' - SV анимация включена (SV Actor)');
    } else if (note.match(/<sv character>/i)) {
        this._svAnimated = true;
        this._isCharacter = true;
        this._hasShadow = true;
        ACTPO.EA.log(enemy.name + ' - SV анимация включена (Character Sprite)');
    }

    if (note.match(/<character index:\s*(\d+)>/i)) {
        this._characterIndex = parseInt(RegExp.$1);
    } else {
...
Sprite_EnemySV.prototype.updateBitmap = function () {
    ...
    if (this._actor) {
        ...
        if (this._actor._isCharacter) {
            this._mainSprite.bitmap = ImageManager.loadCharacter(name, hue);
            // Используем стандартный негативный скейл (флип), как у всех SV противников
...
Sprite_EnemySV.prototype.updateFrame = function () {
    Sprite_Battler.prototype.updateFrame.call(this);
    var bitmap = this._mainSprite.bitmap;
    if (bitmap && bitmap.isReady()) {
        if (this._actor && this._actor._isCharacter) {
            // Логика для character-спрайта (3x4 в составе листа 12x8)
            var big = ImageManager.isBigCharacter(this._actor.battlerName());
            var pw = big ? bitmap.width / 3 : bitmap.width / 12;
            var ph = big ? bitmap.height / 4 : bitmap.height / 8;

            // Защита от "палочки": если расчет выдал слишком маленькие значения, используем стандарт
            if (pw < 1 || isNaN(pw)) pw = 48;
            if (ph < 1 || isNaN(ph)) ph = 48;

            var sx = (this._actor._characterIndex % 4 * 3 + (this._pattern || 0)) * pw;
            // Направление "Вправо" (индекс 2) в стандартном листе персонажа.
            // Т.к. враг стоит слева, он должен смотреть вправо (на игрока).
            var sy = (Math.floor(this._actor._characterIndex / 4) * 4 + 2) * ph;

            this._mainSprite.setFrame(sx, sy, pw, ph);

        } else {
            // Стандартная логика SV
